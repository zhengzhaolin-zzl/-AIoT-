...
unsigned long int k(int t)
{
    ...
    if (t < 20)			c = 0x5a827999;
    else if (t < 40)	c = 0x6ed9eba1;
    else if (t < 60)	c = 0x8f1bbcdc;
    else if (t < 80)	c = 0xca62c1d6;
    return c;
}
 
unsigned long int rotr(int bits, unsigned long int a)
{
	unsigned long int c = 0, d = 0, e = 0, f = 0, g = 0;
    c = (0x0001 << bits) - 1;
    d = ~c;
    e = (a & d) >> bits;
    f = (a & c) << (32 - bits);
...
void sha1(unsigned char *message, int message_length, unsigned char *digest)
{
	...
	unsigned char block_remainder 	= 0;
	unsigned char padded_length 	= 0;
	unsigned char t 				= 0;
	unsigned short int l 			= 0;
	unsigned long int h[5] 		= {0};
	unsigned long int *pw 			= NULL;
	unsigned long int temp 		= 0;
	unsigned long int a = 0, b = 0, c = 0, d = 0, e = 0;
	...
	pw = (unsigned long int *)malloc(80 * 4);
    ...
    for (i = 0; i < num_blocks; i++)
    {
        ...
        for (t=0; t < 80; t++)
        {
            if (t < 16)
            {
                pw[t] =  (256 * 256 * 256) 	* message[(i * 64) + (t * 4)];
                pw[t] += (256 * 256)		* message[(i * 64) + (t * 4) + 1];
                pw[t] += (256) 				* message[(i * 64 )+ (t * 4) + 2];
                pw[t] +=                 	  message[(i * 64) + (t * 4) + 3];
            }
            else if (t < 80)
            {
...
{
    unsigned char b = 64; /* blocksize */
    unsigned char ipad				= 0x36;
    unsigned char opad 			= 0x5c;
    unsigned char k0[64] 			= {0};
    unsigned char k0xorIpad[64] 	= {0};
    unsigned char step7data[64] 	= {0};
    unsigned char i 				= 0;
    unsigned char step8data[64+20] = {0};
	unsigned char step5data[64+64] = {0};
